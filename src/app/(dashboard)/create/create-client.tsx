...
import { createLeague, createTeam } from "@/lib/supabase/mutations";
...
export interface JoinLeagueOption {
  id: string;
  name: string;
  meta: string;
  initials: string;
...
const leagueSteps = ["Basics", "Structure", "Rules"];
...
function FormInput({ label, placeholder, value, onChange, type = "text", className, prefix }: {
  label: string; placeholder: string; value: string; onChange: (v: string) => void; type?: string; className?: string; prefix?: string;
}) {
  return (
...
function SegmentButton({ label, sub, active, onClick }: { label: string; sub?: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={cn("flex-1 text-center py-[13px] px-2 rounded-[11px] border-[1.5px] cursor-pointer transition-colors",
      ...
      {ALL_DAYS.map((d) => {
        ...
        return (
          <button key={d} onClick={() => onToggle(d)}
            className={cn("w-[46px] text-center py-2.5 rounded-[10px] border-[1.5px] font-bold text-[12.5px] cursor-pointer transition-colors",
    ...
    <div className="mb-[26px] flex flex-col gap-2.5 sm:flex-row">
      {([
        ...
        <button key={t.id} onClick={() => onTabChange(t.id)}
          className={cn("flex-1 flex items-center gap-[13px] p-4 px-[18px] rounded-[14px] border-[1.5px] cursor-pointer transition-colors",
        ...
        return (
          <div key={label} className="flex-1 flex flex-col items-center">
            <div className="flex items-center w-full">
              ...
              <div className={cn("w-[30px] h-[30px] rounded-full shrink-0 flex items-center justify-center font-heading font-extrabold text-[13px] border-2",
                done && "bg-accent border-accent text-accent-darker", active && "bg-accent/[0.12] border-accent text-accent", !done && !active && "bg-input border-white/[0.12] text-dim")}>
                {done ? (
            ...
            <div className={cn("text-[11.5px] font-semibold mt-[9px] text-center", active ? "text-primary" : done ? "text-secondary" : "text-dim")}>{label}</div>
          </div>
        );
      })}
    </div>
...
function LeagueStep0({ state, update }: { state: LeagueState; update: (p: Partial<LeagueState>) => void }) {
  ...
  useEffect(() => {
    if (!slugTouched && state.name) update({ slug: toSlug(state.name) });
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <FormInput label="League URL" placeholder="sunday-city-league" prefix="formaxi.app/" value={state.slug}
      ...
      <div>
        ...
        <div className="flex gap-2">
          {([{ v: "5" as const, label: "5-a-side", sub: "Futsal / small" }, { v: "7" as const, label: "7-a-side", sub: "Mid-size" }, { v: "11" as const, label: "11-a-side", sub: "Full pitch" }]).map((o) => (
      ...
      <div>
        ...
        <textarea rows={3} value={state.desc} onChange={(e) => update({ desc: e.target.value })} placeholder="Tell teams what your league is about"
  ...
  return (
    <div className="flex flex-col gap-[22px]">
      <div>
        ...
        <p className="text-[12px] text-muted mt-2">This sets the division&apos;s team cap. Teams join after launch — none are created now.</p>
      </div>
      <div>
...
function LeagueStep2({ state, update }: { state: LeagueState; update: (p: Partial<LeagueState>) => void }) {
  return (
    <div className="flex flex-col gap-[22px]">
      ...
      <div className="flex items-start gap-3 p-3.5 bg-accent/[0.06] border border-accent/[0.18] rounded-xl mt-1">
        <span className="text-[22px] leading-none">✅</span>
        <div className="text-[13px] text-[#bfe9cf] leading-[1.5]">
          Launching creates your league with an opening season — no teams or fixtures yet. Invite or approve teams afterwards, then generate the fixture schedule from the league dashboard when you&apos;re ready.
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col gap-5">
      <div className="flex gap-4 items-end">
        ...
        <div className="w-[110px]">
          ...
          <input value={state.short} onChange={(e) => update({ short: e.target.value.toUpperCase().slice(0, 3) })} maxLength={3} placeholder="RF"
            className="w-full bg-input border border-white/[0.09] rounded-[10px] px-3.5 py-3 text-primary text-sm font-heading font-bold text-center tracking-[1px] outline-none focus:border-accent transition-colors placeholder:text-dim" />
        </div>
      </div>
      <div>
        ...
        <div className="flex gap-[11px] flex-wrap">
          {CREST_COLORS.map((c) => (
            <button key={c} onClick={() => update({ color: c })} className="w-9 h-9 rounded-[10px] cursor-pointer outline-offset-2 transition-shadow"
              style={{ backgroundColor: c, outline: state.color === c ? "2px solid #34e07f" : "2px solid transparent" }} />
          ))}
        </div>
      </div>
    </div>
...
function TeamStep2({ state, update, leagues }: { state: TeamState; update: (p: Partial<TeamState>) => void; leagues: JoinLeagueOption[] }) {
  const options = [...leagues, { id: "none", name: "Skip for now", meta: "Create the team and join a league later", initials: "—", bg: "#33433a" }];
  return (
    <div className="flex flex-col gap-2.5">
      {leagues.length === 0 && (
        <p className="text-[13px] text-muted mb-1">No public leagues to join yet — you can skip and join one later.</p>
      )}
      {options.map((l) => {
        const sel = state.league === l.id;
        return (
          <button key={l.id} onClick={() => update({ league: l.id })}
            className={cn("flex items-center gap-[13px] p-3.5 rounded-[13px] border-[1.5px] cursor-pointer transition-colors", sel ? "bg-accent/[0.06] border-accent" : "bg-input border-white/[0.08]")}>
  ...
  return (
    <div className="flex flex-col gap-[18px]">
      <div>
        ...
        <div className="flex gap-2">
          <input value={inviteDraft} onChange={(e) => setInviteDraft(e.target.value)} onKeyDown={(e: KeyboardEvent) => { if (e.key === "Enter") { e.preventDefault(); addInvite(); } }} placeholder="player@email.com"
            className="flex-1 bg-input border border-white/[0.09] rounded-[10px] px-3.5 py-3 text-primary text-sm font-body outline-none focus:border-accent transition-colors placeholder:text-dim" />
        ...
        <p className="text-[12px] text-muted mt-1.5">Only emails of existing FormaXI users are invited; others can join later.</p>
      </div>
...
// ── Live preview sidebars ───────────────────────────────────────

function LeaguePreview({ state }: { state: LeagueState }) {
  const startNice = (() => {
    try { return state.start ? new Date(state.start + "T00:00").toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "Not set"; }
  ...
  const rows: [string, string][] = [
    ...
    ["Tiebreak", state.tiebreak === "gd" ? "Goal difference" : "Head-to-head"],
      ...
      <div className="bg-gradient-to-br from-[#13261a] to-[#0f1b13] border border-accent/[0.16] rounded-[18px] p-5 overflow-hidden">
        ...
        <div className="flex items-center gap-3 my-3.5 mb-4">
          <div className="w-12 h-12 rounded-[13px] bg-gradient-to-br from-accent to-accent-dark flex items-center justify-center shrink-0">
            <div className="w-[22px] h-[22px] border-[2.6px] border-[#07140c] rounded-full relative flex items-center justify-center"><div className="absolute w-[2.6px] h-[22px] bg-[#07140c]" /></div>
          </div>
        ...
        {rows.map(([label, value]) => (
          <div key={label} className="flex items-start justify-between py-[9px] border-t border-white/[0.06] gap-3">
            ...
            <span className={cn("font-semibold text-[12.5px] text-right break-all", label === "URL slug" && "text-accent")}>{value}</span>
          </div>
        ))}
      </div>
    </div>
...
function TeamPreview({ state, leagues }: { state: TeamState; leagues: JoinLeagueOption[] }) {
  const selectedLeague = leagues.find((l) => l.id === state.league);
  const rows: [string, string][] = [
    ["Abbreviation", state.short || "—"],
    ["Home", state.venue || "Not set"],
    ["Match days", state.days.length ? state.days.join(", ") : "Not set"],
    ["League", selectedLeague?.name ?? (state.league === "none" ? "Skip for now" : "Not selected")],
...
export function CreateClient({ joinableLeagues }: { joinableLeagues: JoinLeagueOption[] }) {
  ...
  const [league, setLeague] = useState<LeagueState>({
    name: "", slug: "", format: "11", region: "", desc: "",
    teams: 10, divisions: 1, rounds: "double", days: [], start: "",
    win: 3, draw: 1, tiebreak: "gd", visibility: "public",
  });

  const [team, setTeam] = useState<TeamState>({
    name: "", short: "", color: CREST_COLORS[0], venue: "", days: [], founded: "", league: "", invites: [],
  });
  ...
  const stepTitles = tab === "league"
    ? ["League basics", "Competition structure", "Rules & scoring"]
    : ["Team identity", "Home ground", "Choose a league", "Build your squad"];
  const stepSubs = tab === "league"
    ? ["Name your competition and pick a format", "Set the size and shape of the season", "Define how points and ranking work, then launch"]
    ...
    startTransition(async () => {
      if (tab === "league") {
        const res = await createLeague({
          name: league.name, slug: league.slug, format: league.format, region: league.region, desc: league.desc,
          divisions: league.divisions, rounds: league.rounds, days: league.days, start: league.start,
          win: league.win, draw: league.draw, tiebreak: league.tiebreak, visibility: league.visibility, teams: league.teams,
        });
        if (res?.error) setError(res.error);
      } else {
        const res = await createTeam({
          name: team.name, short: team.short, color: team.color, venue: team.venue, founded: team.founded,
          leagueId: team.league, invites: team.invites,
        });
        if (res?.error) setError(res.error);
      }
    });
  ...
  const renderStep = () => {
    if (tab === "league") {
      ...
      switch (step) {
        ...
        case 2: return <TeamStep2 state={team} update={updateTeam} leagues={joinableLeagues} />;
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <p className="text-muted text-sm mt-1.5">Set up a new competition or register your team — we&apos;ll walk you through it.</p>

      <div className="mt-[22px]">
        <TabSelector activeTab={tab} onTabChange={(t) => { setTab(t); setStep(0); setInviteDraft(""); setError(null); }} />
      </div>
      ...
      <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[1fr_318px]">
        <Card className="flex flex-col">
          ...
          {renderStep()}
          {error && <p className="text-[12.5px] text-loss font-semibold mt-4">{error}</p>}
          <div className="flex justify-between items-center mt-7 pt-[22px] border-t border-white/[0.06]">
            ...
            <Button variant="primary" disabled={submitting} onClick={() => (isLast ? submit() : setStep(step + 1))}>
              {isLast ? (submitting ? "Working…" : tab === "league" ? "Launch league" : "Create team") : "Continue"}
            </Button>
          </div>
        </Card>
        {tab === "league" ? <LeaguePreview state={league} /> : <TeamPreview state={team} leagues={joinableLeagues} />}
      </div>
    </div>
  );
}
