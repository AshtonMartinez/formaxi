...
function NumberStepper({ value, onChange, min, max, label }: {
  ...
  return (
    <div className="flex-1 bg-input border border-white/[0.08] rounded-xl p-3.5 text-center">
      {label && (
        <div className="text-[11px] text-dim font-bold uppercase tracking-[0.5px]">{label}</div>
      )}
      <div className="flex items-center justify-center gap-3 mt-2">
        <button
          onClick={() => onChange(Math.max(min, value - 1))}
          className="w-7 h-7 rounded-lg bg-white/[0.05] text-heading text-[17px] flex items-center justify-center cursor-pointer hover:bg-white/[0.10] transition-colors"
        >
          −
        </button>
        ...
        <button
          ...
          className="w-7 h-7 rounded-lg bg-white/[0.05] text-heading text-base flex items-center justify-center cursor-pointer hover:bg-white/[0.10] transition-colors"
        >
          +
        </button>
      </div>
    ...
    <div className="flex flex-col gap-2">
      {invites.map((email, i) => (
        <div key={i} className="flex items-center gap-[11px] px-3 py-2.5 bg-input rounded-[10px] border border-white/[0.05]">
          <span className="w-[30px] h-[30px] rounded-lg bg-[#1b2820] flex items-center justify-center font-heading font-extrabold text-[11px] text-accent shrink-0">
            {email.slice(0, 2).toUpperCase()}
          </span>
          <span className="flex-1 text-[13.5px] text-heading whitespace-nowrap overflow-hidden text-ellipsis">
            {email}
          </span>
          <span className="text-[11px] text-draw font-semibold">{statusLabel}</span>
          <button
            onClick={() => onRemove(i)}
            className="w-6 h-6 rounded-[7px] text-dim text-base flex items-center justify-center cursor-pointer hover:text-loss hover:bg-loss/[0.1] transition-colors"
          >
            ×
          </button>
        </div>
  ...
  return (
    <div className="flex gap-2.5 mb-[26px]">
      {([
        ...
        >
          <span className={cn("w-[38px] h-[38px] rounded-[11px] flex items-center justify-center shrink-0 text-lg", t.iconBg)}>
            {t.icon}
          </span>
    ...
    <div className="flex flex-col gap-[22px]">
      <div>
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => update({ teams: Math.max(2, teamsN - 1) })}
            className="w-10 h-10 rounded-[11px] bg-input border border-white/[0.12] text-heading text-[22px] flex items-center justify-center cursor-pointer hover:bg-white/[0.06] transition-colors"
          >
            −
          </button>
          <div className="font-heading font-black text-[28px] min-w-[44px] text-center">{teamsN}</div>
          <button
            onClick={() => update({ teams: Math.min(30, teamsN + 1) })}
            className="w-10 h-10 rounded-[11px] bg-input border border-white/[0.12] text-heading text-xl flex items-center justify-center cursor-pointer hover:bg-white/[0.06] transition-colors"
          >
            +
          </button>
          <div className="text-dim text-[12.5px] leading-[1.4]">
            {totalMatches} matches across<br />{totalRounds} rounds
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <DayChips
          selected={state.days}
          onToggle={(d) => update({ days: state.days.includes(d) ? state.days.filter((x) => x !== d) : [...state.days, d] })}
        />
      </div>
...
function LeagueStep2({ state, update }: { state: LeagueState; update: (p: Partial<LeagueState>) => void }) {
  return (
    <div className="flex flex-col gap-[22px]">
      ...
      <button
        onClick={() => update({ autoFix: !state.autoFix })}
        className="flex items-center gap-[13px] p-3.5 bg-input rounded-xl border border-white/[0.07] cursor-pointer"
      >
        <span
          className={cn(
            "inline-flex w-10 h-[23px] rounded-full p-0.5 shrink-0 transition-colors",
            state.autoFix ? "bg-accent justify-end" : "bg-[#33433a] justify-start",
          )}
        >
...
function LeagueStep3({
  ...
  state: LeagueState;
  update: (p: Partial<LeagueState>) => void;
  inviteDraft: string;
  setInviteDraft: (v: string) => void;
}) {
  const addInvite = () => {
  ...
  return (
    <div className="flex flex-col gap-[18px]">
      <div>
        ...
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={(e: KeyboardEvent) => { if (e.key === "Enter") { e.preventDefault(); addInvite(); } }}
          ...
          />
          <button onClick={addInvite} className="bg-accent/[0.12] text-accent font-bold text-[13.5px] px-[18px] rounded-[10px] flex items-center cursor-pointer hover:bg-accent hover:text-accent-darker transition-colors">
            Add
          </button>
        </div>
      </div>
      ...
      <InviteList
        ...
        onRemove={(i) => update({ invites: state.invites.filter((_, idx) => idx !== i) })}
        statusLabel="Pending"
      />
    ...
    <div className="flex flex-col gap-5">
      <div className="flex gap-4 items-end">
        ...
        <div className="w-[110px]">
          ...
          <input
            value={state.short}
            onChange={(e) => update({ short: e.target.value.toUpperCase().slice(0, 3) })}
            maxLength={3}
            placeholder="RF"
        ...
        <div className="flex gap-[11px] flex-wrap">
          {CREST_COLORS.map((c) => (
            <button
              ...
              className="w-9 h-9 rounded-[10px] cursor-pointer outline-offset-2 transition-shadow"
    ...
    <div className="flex flex-col gap-2.5">
      {joinLeagues.map((l) => {
        ...
        return (
          <button
            key={l.id}
            onClick={() => update({ league: l.id })}
            className={cn(
              "flex items-center gap-[13px] p-3.5 rounded-[13px] border-[1.5px] cursor-pointer transition-colors",
              sel ? "bg-accent/[0.06] border-accent" : "bg-input border-white/[0.08]",
            )}
          >
            ...
            <span
              className={cn(
                ...
                sel ? "border-accent bg-accent" : "border-white/[0.25] bg-white/[0.18]",
              )}
            />
          </button>
        );
...
function TeamStep3({
  ...
  state: TeamState;
  update: (p: Partial<TeamState>) => void;
  inviteDraft: string;
  setInviteDraft: (v: string) => void;
}) {
  const addInvite = () => {
      ...
      <div className="bg-gradient-to-br from-[#13261a] to-[#0f1b13] border border-accent/[0.16] rounded-[18px] p-5 overflow-hidden">
        ...
        <div className="flex items-center gap-3 my-3.5 mb-4">
          ...
          <div className="min-w-0">
            <div className="font-heading font-extrabold text-base whitespace-nowrap overflow-hidden text-ellipsis">
              {state.name || "Untitled League"}
            </div>
        ...
        {rows.map(([label, value]) => (
          <div key={label} className="flex items-center justify-between py-[9px] border-t border-white/[0.06]">
            ...
            <span className={cn("font-semibold text-[12.5px]", label === "Invited" && "text-accent")}>{value}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="sticky top-[88px]">
      <div className="bg-gradient-to-br from-[#1a2030] to-[#0f1b13] border border-white/[0.1] rounded-[18px] p-5 overflow-hidden">
        ...
        <div className="flex flex-col items-center my-4 mb-[18px]">
          <div
            className="w-[66px] h-[66px] rounded-[18px] flex items-center justify-center font-heading font-black text-[22px] text-white shadow-[0_8px_22px_rgba(0,0,0,0.35)]"
            style={{ backgroundColor: state.color }}
          >
            {state.short || "?"}
          </div>
          <div className="font-heading font-extrabold text-[17px] mt-3 text-center">
            {state.name || "Untitled Team"}
          </div>
        </div>
        {rows.map(([label, value]) => (
          <div key={label} className="flex items-center justify-between py-[9px] border-t border-white/[0.06]">
            <span className="text-muted text-[12.5px]">{label}</span>
            <span className={cn(
              "font-semibold text-[12.5px] max-w-[150px] text-right whitespace-nowrap overflow-hidden text-ellipsis",
              label === "Players invited" && "text-accent",
            )}>{value}</span>
          </div>
        ))}
      </div>
...
export default function CreatePage() {
  ...
  const [league, setLeague] = useState<LeagueState>({
    name: "Sunday City League", format: "11", region: "Hackney Marshes, London",
    desc: "Competitive 11-a-side Sunday football for East London clubs.",
    teams: 10, rounds: "double", days: ["Sun"], start: "2025-09-07",
  ...
  return (
    <div className="p-[28px_30px_56px] max-w-[1040px] mx-auto">
      <h1 className="font-heading font-black text-[26px] tracking-[-0.5px]">Create</h1>
      ...
      <div className="grid grid-cols-[1fr_318px] gap-[22px] items-start">
